...
export const updateVideo = catchAsync(async (req: AuthRequest, res: Response) => {
  if (!req.user) {
    throw new AppError(401, 'Not authenticated');
  }

  const video = await videoService.updateVideo(req.user, req.params.videoId, req.body);
...
export const deleteVideo = catchAsync(async (req: AuthRequest, res: Response) => {
  if (!req.user) {
    throw new AppError(401, 'Not authenticated');
  }
